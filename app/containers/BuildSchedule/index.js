...
export class BuildSchedule extends React.Component {
  ...
  addAgenda() {
    ...
    const nameCheck =
      ...
      this.state.agendaModule.name === null;
    const dayCheck =
      this.state.agendaModule.schedule.day === "" ||
      this.state.agendaModule.schedule.day === null;
    const startTimeCheck = timeChecker.test(
    ...
    const roomCheck =
      ...
      this.state.agendaModule.schedule.room === null;
    if (
      startTimeCheck &&
      endTImeCheck &&
      !nameCheck &&
      ...
      this.props.addSelectedClass(
        ...
        } - ${this.state.agendaModule.schedule.room}`,
        {
          ...
          schedule_items: [
            {
              day: this.state.agendaModule.schedule.day,
              start: this.state.agendaModule.schedule.start,
              end: this.state.agendaModule.schedule.end,
              room: this.state.agendaModule.schedule.room
            }
          ],
          credit: this.state.agendaModule.sks
        }
      );
      ...
      if (!startTimeCheck) {
        newState["errMesage"]["schedule"]["start"] =
          "Error, format jam salah, seharusnya HH.MM, contoh: 12.30";
      }
  ...
  render() {
    ...
    if (!isEmpty(this.props.localState.courses)) {
      scheduleListItems = (
        <List items={this.props.localState.courses} component={Course} />
      );
    }
    ...
    if (!isEmpty(this.props.localState.picked)) {
      for (let [key, value] of Object.entries(this.props.localState.picked)) {
        ...
        const entry = (
          <div
            key={`pickedcourse-${key}`}
            className="small-12 columns"
            style={{ background: isConflict ? "#CE9D4D" : "none" }}
          >
            <div className="row expanded">
              ...
              <div className="small-5 columns">
                <div className={styles.tableItem}>
                  <ul className={styles.time}>{classesTimes}</ul>
                </div>
              </div>
              ...
              <div className="small-1 columns text-right">
                <button
                  ...
                  onClick={() => this.props.removePicked(key)}
                  style={{ color: isConflict ? "inherit" : "#CE9D4D" }}
                >
                  X
                </button>
              </div>
              ...
              <div className="row expanded">
                ...
                <div className="small-12 columns text-right">
                  <button
                    ...
                    onClick={this.finishAddAgenda}
                  >
                    BATAL
                  </button>
                  <button className={styles.add} onClick={this.addAgenda}>
                    SIMPAN
                  </button>
                </div>
              </div>
        ...
        <div className="row expanded">
          <div className="small-12 medium-9 columns">
            <div className={styles.scheduleList}>
              ...
              <div className="row expanded">{scheduleListItems}</div>
            </div>
          </div>
          <div className="small-12 medium-3 columns">
            <div className={styles.pickedSchedule}>
              <div className="row expanded">
                <div className="small-12 columns">
                  <div className={styles.pickedScheduleHeader}>
                    Kelas Pilihan
                  </div>
                </div>
                <div className="small-12 columns">
                  ...
                  <div className={styles.pickedScheduleList}>
                    <div className="row expanded">
                      ...
                      <div
                        ...
                        style={{
                          display:
                            this.props.localState.conflict.length > 0
                              ? "block"
                              : "none"
                        }}
                      >
                ...
                <div className="small-12 columns">
                  <button
                    ...
                    disabled={
                      this.props.localState.conflict.length > 0 ||
                      totalCredits > 24 ||
                      isEmpty(this.props.localState.picked)
                    }
                  >
                    Simpan Jadwal
                  </button>
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    changeSelectedClass: (coursename, payload) =>
      dispatch(changeSelectedClass(coursename, payload)),
    addSelectedClass: (coursename, payload) =>
      dispatch(addSelectedClass(coursename, payload)),
