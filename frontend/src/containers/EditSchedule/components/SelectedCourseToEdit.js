...
import { isScheduleConflict } from "../../SelectedCourses/utils";
import TrashIcon from "assets/Trash.png";
import TrashWhiteIcon from "assets/TrashWhite.png";
import Agenda from "../../SelectedCourses/Agenda";
...
function SelectedCourseToEdit({ history, scheduleId }) {
  ...
  const handleDeleteSchedule = async () => {
    dispatch(setLoading(true));
    const res = await makeAtLeastMs(deleteSchedule(auth.userId, scheduleId), 1000);
  ...
  return (
    <React.Fragment>
      ...
      <Container>
        ...
        <Button
          onClick={() => totalCredits === 0 ? handleDeleteSchedule() : saveSchedule()}
          disabled={isConflict || totalCredits > 24}
          intent={totalCredits === 0 && 'danger'}
        >
...
export default withRouter(SelectedCourseToEdit);
